...
async def test_reconnect_respects_max_attempts():
    ...
    with patch.object(
        ...
        # third attempt should be a no-op due to the cap
        await manager.reconnect_with_backoff(0)
        assert manager.get(0).reconnect_attempts == 2
...
async def test_poll_telemetry_noop_when_offline():
    manager = _manager()
    # machine starts OFFLINE; poll should do nothing and not raise
...
def test_backoff_delay_grows_and_caps():
    ...
    assert manager.next_backoff_delay(0) == 10.0  # capped
