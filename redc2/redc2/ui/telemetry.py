...
_REMOTE_PROBE = r"""
REDC2_PY_BIN=""
for _cand in python3 python3.13 python3.12 python3.11 python3.10 python3.9 /usr/bin/python3 /usr/local/bin/python3; do
    if command -v "$_cand" >/dev/null 2>&1 && "$_cand" -c "import psutil" >/dev/null 2>&1; then
        REDC2_PY_BIN="$_cand"
        break
    fi
done
if [ -z "$REDC2_PY_BIN" ]; then
    REDC2_PY_BIN="python3"
fi

"$REDC2_PY_BIN" - <<'REDC2_PY'
...
try:
    ...
    data["_psutil_error"] = f"{e} (python={sys.executable})"
...
def collect_snapshot(session: SSHSession) -> TelemetrySnapshot:
    ...
    if raw.get("_psutil_error"):
        logger.warning("psutil unavailable on remote host: %s", raw["_psutil_error"])
