"""REDC2 Textual application - COMPLETE POLISHED VERSION."""
...
class REDC2App(App):
    """The REDC2 dashboard and terminal application."""
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="main-container"):
            ...
            # Terminal will be added when a machine is selected
    ...
    async def _telemetry_loop(self) -> None:
        """Periodically poll telemetry for every online machine."""
    ...
    async def _reconnect_loop(self) -> None:
        """Background auto-reconnect with per-machine exponential backoff."""
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        if not text.strip():
            event.input.value = ""
            return
        # Show what user typed in the log
    ...
    def _handle_result(self, raw_text: str, result: CommandResult) -> None:
        log = self.query_one("#output-log", RichLog)
        # Command already logged in on_input_submitted, just show result
        if result.message:
    ...
    def _create_terminal_widget(self) -> None:
        """Create SSH terminal widget for the selected machine."""
        ...
        # Widget.remove() is async - it only *schedules* removal, it doesn't
        # happen immediately. Mounting a new widget with the same id right
        # after calling remove() races against that and raises DuplicateIds.
        # Give each terminal instance a unique id instead of reusing
        # "ssh-terminal", and let remove() clean up the old one in the
        # background.
