"""MachineManager: the single source of truth for known machines.

The manager owns the list of :class:`Machine` instances and their
associated :class:`SSHSession` objects. UI code and the command layer
talk to machines only through this manager — never directly to Paramiko.

Connection/reconnection is asynchronous: Paramiko itself is blocking, so
all network calls are pushed to a worker thread via `asyncio.to_thread`,
...
class MachineNotFoundError(Exception):
    """Raised when a machine id does not exist."""
...
class MachineManager:
    """Owns and indexes all configured machines and their SSH sessions."""
    ...
    def list_machines(self) -> list[Machine]:
        """Return machines ordered by id (0-indexed, stable)."""
    ...
    def add_machine(self, machine_config: MachineConfig) -> Machine:
        """Register a new machine at the next available id."""
    ...
    async def connect(self, machine_id: int) -> None:
        """Connect (or reconnect) a single machine. Never raises.

        Updates the machine's state/last_error in place; callers that
        need to know the outcome should inspect `machine.state` after
        awaiting this.
        """
        ...
        async with lock:
            ...
            except Exception as exc:  # noqa: BLE001 - never let a bad machine crash the app
    ...
    async def reconnect_with_backoff(self, machine_id: int) -> None:
        """Attempt one reconnect, respecting the configured attempt cap.

        Backoff itself (the delay between calls) is the caller's
        responsibility, typically a periodic background worker; this
        method only enforces the maximum-attempts ceiling and performs a
    ...
    def next_backoff_delay(self, machine_id: int) -> float:
        """Compute the next exponential backoff delay for a machine."""
        machine = self.get(machine_id)
    ...
    async def poll_telemetry(self, machine_id: int) -> None:
        """Collect one telemetry snapshot for a machine. Never raises.

        If the machine isn't connected, this is a no-op. If the SSH
        session errors out mid-poll, the machine is marked offline/error
        so the periodic reconnect logic can pick it back up.
        """
        machine = self.get(machine_id)
        session = self.get_session(machine_id)
    ...
    async def shutdown(self) -> None:
        """Close all SSH sessions cleanly."""
