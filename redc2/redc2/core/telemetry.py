"""Telemetry collection: pulls system stats from a remote machine over SSH.

We don't require psutil to be installed on the remote machine. Instead we
run small, portable shell/python snippets over the existing SSH session
and parse the output defensively — any missing sensor becomes `None` and
is rendered as "N/A" by the UI layer, never a crash.
"""
...
# A single portable python3 one-liner run remotely. It relies only on the
# standard library (plus psutil if present) so it works on a stock Debian
# or Raspberry Pi OS install without extra setup.
_REMOTE_PROBE = r"""
python3 - <<'REDC2_PY'
...
def collect_snapshot(session: SSHSession) -> TelemetrySnapshot:
    """Run the remote probe and parse it into a :class:`TelemetrySnapshot`.

    Never raises for missing/malformed individual fields; only raises
    SSHConnectionError if the session itself is unusable (propagated so
    the caller can mark the machine offline/error and retry later).
    """
...
def parse_telemetry_json(raw_line: str) -> dict:
    """Small standalone parser used by unit tests to validate JSON shape."""
    import json
