"""Configuration loading for REDC2.

Configuration is stored in a TOML file (default: ``config/machines.toml``).
Each machine entry describes how to reach it over SSH. Plaintext passwords
are never stored in configuration; authentication is via SSH key, SSH
agent, or an interactive password prompt at connect time.
"""
...
class MachineConfig:
    """Static configuration describing how to reach one machine over SSH."""
...
def find_default_config() -> Path | None:
    """Look for a machines.toml in common locations, returning the first hit."""
    candidates = [
